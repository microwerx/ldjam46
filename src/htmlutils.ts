...
function createRangeRow(
    parent: HTMLElement, id: string, curValue: number, minValue: number,
    maxValue: number, stepValue = 1, isvector = false) {
...
function createButtonRow(
    parent: HTMLElement, id: string, caption: string, callback: () => void) {
  let lContent = '<div class=\'column left\'><label for=\'' + id + '\'>' + id +
      '<label></div>';
...
function getRangeVector3(id: string): Vector3 {
  return Vector3.make(
      getRangeValue(id + '1'), getRangeValue(id + '2'),
      getRangeValue(id + '3'));
}
